    ...
    public class ProjectService : IProject
    {
        ...
        public async Task<APIResponse> AddEditProjectJobDetail(ProjectJobDetailModel model, string UserId)
        {
            ...
            try
            {
                if (model.ProjectId != 0)
                {
                    ...
                    {
                        ProjectJobDetail projectJobDetail = await _uow.GetDbContext().ProjectJobDetail.FirstOrDefaultAsync(x => x.IsDeleted == false && x.ProjectJobId == model.ProjectJobId);
        ...
        public async Task<APIResponse> GetSignedURL()
        {
            ...
            try
            {
                ...
                //UrlSigner urlSigner = UrlSigner.FromServiceAccountPath(googleApplicationCredentail);
                //string url = urlSigner.Sign(
                //    bucketName,
                //    "Voucher/00001/lost.docx",
                ...
                // signedURL = await GCBucket.GetSignedURL(url);

                UrlSigner urlSigner = UrlSigner.FromServiceAccountPath(googleApplicationCredentail);
                var destination = "alpitji.docx";
