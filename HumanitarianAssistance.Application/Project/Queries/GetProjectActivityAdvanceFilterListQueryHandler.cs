        ...
        public async Task<ApiResponse> Handle(GetProjectActivityAdvanceFilterListQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var spActivityList = await _dbContext.LoadStoredProc("get_project_projectactivitylist_filter")
                                          ...
                                          .WithSqlParam("planned_start_date",  request.PlannedStartDate == null ? string.Empty : request.PlannedStartDate.Value.ToString("yyyy-MM-dd"))
                                          .WithSqlParam("planned_end_date", request.PlannedEndDate == null ? string.Empty : request.PlannedEndDate.Value.ToString("yyyy-MM-dd"))
                                          .WithSqlParam("actual_start_date", request.ActualStartDate == null ? string.Empty : request.ActualStartDate.Value.ToString("yyyy-MM-dd"))
                                          .WithSqlParam("actual_end_date", request.ActualEndDate == null ? string.Empty : request.ActualEndDate.Value.ToString("yyyy-MM-dd"))
