        ...
        public async Task<byte[]> Handle (EmployeeAnnualTunoverReportPdfQuery request, CancellationToken cancellationToken) {
            try {
                 var exitInterviewDetails = (from eid in _dbContext.ExistInterviewDetails
                    ...
                    on eid.EmployeeID equals ed.EmployeeID into edl from ed in edl.DefaultIfEmpty ()
                    join epd in _dbContext.EmployeeProfessionalDetail 
                    on ed.EmployeeID equals epd.EmployeeId into epdl from epd in epdl.DefaultIfEmpty ()
                    join d in _dbContext.DesignationDetail 
                    on epd.DesignationId equals d.DesignationId into dl from d in dl.DefaultIfEmpty ()  
                    join o in _dbContext.OfficeDetail 
                    on eid.OfficeId equals o.OfficeId into od from o in od.DefaultIfEmpty ()
                    join et in _dbContext.EmployeeType 
                    on ed.EmployeeTypeId equals et.EmployeeTypeId into etl from et in etl.DefaultIfEmpty ()
