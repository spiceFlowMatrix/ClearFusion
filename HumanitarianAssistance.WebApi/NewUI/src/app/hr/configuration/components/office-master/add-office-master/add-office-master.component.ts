...
export class AddOfficeMasterComponent implements OnInit {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<AddOfficeMasterComponent>,
  ...
  ngOnInit() {
    ...
    if (this.data) {
      ...
      this.addOfficeForm
        .get('SupervisorName')
        .patchValue(this.data.SupervisorName);
  ...
  addOffice() {
    ...
    if (this.addOfficeForm.valid) {
      this.hrService.addOffice(this.addOfficeForm.value).subscribe(
        x => {
          if (x.StatusCode === 200) {
  ...
  editOffice() {
    this.isFormSubmitted = true;
    if (this.addOfficeForm.valid) {
      this.hrService.editOffice(this.addOfficeForm.value).subscribe(
        x => {
          if (x) {
            this.toastr.success('Success');
            this.isFormSubmitted = false;
