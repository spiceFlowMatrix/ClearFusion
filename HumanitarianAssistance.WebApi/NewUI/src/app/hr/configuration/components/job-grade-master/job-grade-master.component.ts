...
export class JobGradeMasterComponent implements OnInit {
  ...
  jobGradeHeaders$ = of(['Id', 'Grade Name', 'AccountId', 'Account Associated']);
  hideColums$ = of({ headers: ['Grade Name', 'Account Associated'], items: ['GradeName', 'AccountName'] });
  ...
  getJobGradeList() {
    ...
    this.hrService.getJobGradeList(this.pageModel).subscribe(
      x => {
