...
var InterviewDetailComponent = /** @class */ (function () {
    ...
    InterviewDetailComponent.prototype.actionEventsLanguage = function (event) {
        if (event.type === 'delete') {
            var index = this.interviewDetailForm.controls['LanguageList']
                .value.findIndex(function (x) { return x.LanguageName === event.item.LanguageName; });
            this.interviewDetailForm.controls['LanguageList'].value.splice(index, 1);
        }
    };
    InterviewDetailComponent.prototype.actionEventsTraining = function (event) {
        if (event.type === 'delete') {
            var index = this.interviewDetailForm.controls['TraningList']
                .value.findIndex(function (x) { return x.LanguageName === event.item.LanguageName; });
            this.interviewDetailForm.controls['TraningList'].value.splice(index, 1);
        }
    };
    InterviewDetailComponent.prototype.actionEventsInterviewers = function (event) {
        if (event.type === 'delete') {
            var index = this.interviewDetailForm.controls['InterviewerList']
                .value.findIndex(function (x) { return x.EmployeeId === event.item.EmployeeId; });
            this.interviewDetailForm.controls['InterviewerList'].value.splice(index, 1);
        }
    };
    //#endregion
