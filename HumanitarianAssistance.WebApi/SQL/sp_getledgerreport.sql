CREATE
OR REPLACE FUNCTION public.get_ledger_report(
    currency integer,
    fromdate text,
    todate text,
    recordtype integer,
...
DECLARE _recordtype integer = recordtype;
...
_openingbalance boolean = openingbalance;

BEGIN CREATE temp TABLE temp_ledger_report(
    chartofaccountnewid BIGINT,
    accountname TEXT,
    voucherno BIGINT,
    description TEXT,
...
ORDER BY
    vt."TransactionDate" ASC;

ELSE
INSERT INTO
    temp_ledger_report
SELECT
    tt."ChartOfAccountNewId",
    tt."AccountName",
    tt."VoucherNo",
    tt."Description",
    tt."ReferenceNo",
...
FROM
    (
        ...
        FROM
            ...
            LEFT JOIN "VoucherTransactions" vt ON vd."VoucherNo" = vt."VoucherNo"
            LEFT JOIN "ChartOfAccountNew" coa ON vt."ChartOfAccountNewId" = coa."ChartOfAccountNewId"
            LEFT JOIN "CurrencyDetails" cd ON vd."CurrencyId" = cd."CurrencyId"
        WHERE
            CASE
                _openingbalance
                WHEN true THEN vd."VoucherDate" :: date < _fromdate :: date
