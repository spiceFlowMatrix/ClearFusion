CREATE
OR REPLACE FUNCTION public.get_projectproposalreport(
    ...
    projectsname character varying,
    ...
    duedays integer
) LANGUAGE 'plpgsql' COST 100 VOLATILE ROWS 1000 AS $Body$ DECLARE BEGIN CREATE TEMP TABLE temp_projectproposalreport(
...
INSERT INTO
    temp_projectproposalreport
SELECT
    tt."ProjectName",
    tt."ProjectCode",
    tt."ProposalStartDate",
    tt."ProposalDueDate",
    CASE
        WHEN currencyid <> 0 THEN currencyid
        ELSE tt."CurrencyId"
    END,
    tt.proposalbudget,
    round(CAST(tt.progress as numeric), 2),
    tt.colorcode,
    tt.reviewcompletiondate,
    tt.duedays
FROM
    (
        SELECT
            pd."ProjectName",
            pd."ProjectCode",
            ppd."ProposalStartDate",
            ppd."ProposalDueDate",
            ppd."CurrencyId",
            CASE
                WHEN currencyid <> 0 THEN COALESCE(
                    ppd."ProposalBudget" * (
                        SELECT
                            "Rate"
                        FROM
            ...
            CASE
                WHEN ppd."ProposalStartDate" IS NOT NULL THEN CASE
                    WHEN ppd."ProposalDueDate" IS NOT NULL THEN CASE
                        WHEN apd."IsApproved" = true THEN CASE
                            WHEN (
                                DATE_PART(
                                    'day',
                                    ppd."ProposalDueDate" - ppd."ProposalStartDate"
                                ) * 24 + DATE_PART(
                                    'hour',
                                    ppd."ProposalDueDate" - ppd."ProposalStartDate"
                                )
                                ...
                                (
                                    DATE_PART(
                                        'day',
                                        apd."ReviewCompletionDate" - ppd."ProposalStartDate"
                                    ) * 24 + DATE_PART(
                                        'hour',
                                        apd."ReviewCompletionDate" - ppd."ProposalStartDate"
                                    )
            ...
            CASE
                WHEN ppd."ProposalStartDate" IS NOT NULL THEN CASE
                    WHEN ppd."ProposalDueDate" IS NOT NULL THEN CASE
                        WHEN apd."IsApproved" = true THEN CASE
                            WHEN apd."ReviewCompletionDate" IS NOT NULL THEN CASE
                                WHEN DATE_PART(
                                    'day',
                                    ppd."ProposalDueDate" :: timestamp - ppd."ProposalStartDate" :: timestamp
                                ) > 0 THEN CASE
                                    WHEN (
                                        (
                                            DATE_PART(
                                                'day',
                                                apd."ReviewCompletionDate" :: timestamp - ppd."ProposalStartDate" :: timestamp
                                            ) / DATE_PART(
                                                'day',
                                                ppd."ProposalDueDate" :: timestamp - ppd."ProposalStartDate" :: timestamp
                                            )
                            ...
                            WHEN DATE_PART(
                                ...
                                WHEN (
                                    (
                                        DATE_PART(
                                            'day',
                                            now() :: timestamp - ppd."ProposalStartDate" :: timestamp
                                        ) / DATE_PART(
                                            'day',
                                            ppd."ProposalDueDate" :: timestamp - ppd."ProposalStartDate" :: timestamp
                                        )
            ...
            END AS ColorCode,
            CASE
                WHEN apd."IsApproved" = true THEN apd."ReviewCompletionDate"
                ELSE null
            END as reviewcompletiondate,
            CASE
                WHEN apd."IsApproved" = true THEN CASE
                    WHEN apd."ReviewCompletionDate" IS NOT NULL THEN CASE
                        WHEN apd."ReviewCompletionDate" > ppd."ProposalDueDate" THEN DATE_PART(
                            'day',
                            apd."ReviewCompletionDate" :: timestamp - ppd."ProposalDueDate" :: timestamp
                        )
                        ELSE 0
                    END
                    ELSE 0
                END
                ELSE CASE
                    WHEN ppd."ProposalDueDate" IS NOT NULL THEN DATE_PART(
                        'day',
                        now() :: timestamp - ppd."ProposalDueDate" :: timestamp
                    )
                    ELSE 0
                END
            END AS DueDays
        ...
        WHERE
            ...
            AND (
                islate = false
                OR (
                    now() :: Date > ppd."ProposalDueDate" :: Date
                    AND (
                        iscompleted = true
                        OR apd."IsApproved" = false
                    )
                )
            )
